...
def run():
    ...
    # run the pacstrap
    pacstrap_command = ["/etc/calamares/scripts/pacstrap_calamares", root_mount_point] + base_packages  # СРАНЫЕ Endevaour не могут код нормально написать, нахера мне -c аргумент, НАХЕРА хранить кэш пакетов НА ФЛЕШКЕ? Спокойно можно его очистить ОДНОЙ сраной командой, зато не надо хранить кэш пакетов на флешке, обьем которой ограничен 256 мегабайтами... Капец.
    try:
